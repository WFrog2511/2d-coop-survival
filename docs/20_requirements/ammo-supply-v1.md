...
- 右下のDOM ammo panelに選択武器名、弾倉残数/上限、予備弾薬/上限を表示する。Issue #71追加sliceではTab詳細画面の右隣に、rifle、shotgun、handgunの各`AmmoType`のicon、名称、既存`reserve`だけを表示する弾薬ポーチを併置する。sidearmは同じhandgun行を共有し、弾倉はpouchへ表示しない。既存の診断HUDと右下panelは維持する。
