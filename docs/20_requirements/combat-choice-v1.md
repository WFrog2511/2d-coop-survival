...
prototype-v0を拡張し、初期ライフルとworld pickupで取得するショットガン・ハンドガン、9体の基本敵・3体の高速ドローンを、遮蔽物を持つ自動生成アリーナで選択・識別・撃破できる最小戦闘ループを提供する。

## Issue #71の限定supersede

[Issue #71](https://github.com/WFrog2511/2d-coop-survival/issues/71)は、本書の旧「二つの武器」および「第三武器は対象外」という範囲を履歴として扱い、`WeaponId`のハンドガン、slot 3、`3`キー、shared ammo stateだけを追加する。既存の戦闘選択・敵・地形の契約は維持する。
...
- `1`キーでアサルトライフル、`2`キーでショットガン、`3`キーでハンドガンを切り替え、選択中の武器・残弾・リロード状態をHUDに表示
...
- `R`キーで選択武器を手動リロードする。予備弾薬は武器別の`reserveInitial`、`reserveMax`、`ammoBoxRecovery`で有限に管理し、ハンドガンは`HANDGUN_AMMO`の10発単位を使う。ハンドガンを複数取得しても同じ残弾・予備弾・リロード状態を共有する。リロード中は射撃不可、武器切替はリロードを中断して残弾を保持する
