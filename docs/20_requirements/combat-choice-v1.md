...
- `R`キーで選択武器を手動リロードする。予備弾薬は武器別の`reserveInitial`、`reserveMax`、`ammoBoxRecovery`で有限に管理し、handgun系sidearmは`HANDGUN_AMMO`の10発単位を使う。sidearm modelを複数取得しても同じ残弾・予備弾・リロード状態を共有する。リロード中は射撃不可、異なる`WeaponId`への切替だけがリロードを中断して残弾を保持し、同じhandgunのsidearm model切替は継続する。Tab詳細インベントリを開いている間はpointerdownと自動射撃を無作用にし、弾薬消費と空弾倉の自動リロードを開始しないが、`R`の手動リロードは維持する
