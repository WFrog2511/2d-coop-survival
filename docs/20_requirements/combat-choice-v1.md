...
prototype-v0を拡張し、二つの武器と3体の基本敵・1体の高速ドローンを、遮蔽物を持つ自動生成アリーナで選択・識別・撃破できる最小戦闘ループを提供する。

## 必須範囲

- PCブラウザ上の800×500px表示領域、40pxタイル・40×25マスの広域アリーナと1人プレイ
- 7個の矩形部屋、幅1〜3マスのL字通路、外周wall、到達可能floor、少数の1マス障害物をseed付きで自動生成
...
- 3体の基本敵はHP6、高速ドローンはHP4で同時に表示する。基本敵は両弾種を100%、ドローンは小口径弾を50%・散弾を100%のダメージとして受ける
- 高速ドローンの耐性をHUDへ事前表示し、再現可能な合成横速度で追跡方向に直交するジグザグ接近を行う
- 敵個体ごとのHP、撃破、再出現、接触、ノックバック、命中フィードバックを管理し、耐性が働いた命中は通常命中と区別して表示する。基本敵は1300〜1900ms、高速ドローンは900〜1500msで、seed・敵ID・出現回数に対して決定的に再出現する
