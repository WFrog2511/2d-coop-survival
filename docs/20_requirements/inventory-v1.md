...
- Tab詳細画面の右隣には弾薬ポーチを表示し、rifle、shotgun、handgunの各`AmmoType`のicon、名称、既存`reserve`だけを一覧表示する。弾倉は含めず、ハンドガン、リボルバー、コンパクトピストルは同じhandgun行を共有する。既存右下ammo panelの選択武器名、弾倉、予備弾、reload表示は維持する。詳細画面を開いている間はpointerdownと自動射撃を無作用にし、弾薬消費と空弾倉の自動reloadを始めない。移動、時間進行、`R`の手動reload、pickup、slot選択はnonmodalのまま維持する。
- terminal中は全pickup処理を止め、Tab画面と弾薬ポーチを閉じる。retry / new run は初期クイックスロット、バックパック、全weaponの弾薬・reload状態、world weapon、スクラップ、既存弾薬箱を新しいrunへ初期化し、次のTab表示は初期`reserve`を示す。

## 対象外

field上の個別弾薬item、弾薬の`E`取得、弾薬drag & drop・stack分割、素材のdrag & drop、touch操作、武器の個体別弾倉・耐久・改造、modelごとのbalance差、重量、容量拡張、クラフト、設備、役職固有補正、マルチ同期、永続化、汎用inventory framework、新規依存、balance変更は含めない。drag & drop、クイックスロットとバックパック間の移動・並べ替え、worldへのweapon dropを対象外とした旧記述は#71追加slice以前の履歴である。
...
- quickbarはworld item総数を表示せず、hidden metadataはE2Eから読める。Tab中のpointerdownと長押しは弾薬・reload状態を変えず、閉じると射撃が復帰する。弾薬ポーチは3弾種の`reserve`と同期し、弾薬箱取得とretry後の初期値を表示する。
