...
[REQ-WAVE-PROGRESSION](wave-progression-v1.md)（Issue #71条件変更後）は、Issue #34由来の180000ms（3分）生存期限だけでなく、本書の`run開始`をdirectional spawn epochおよびinitial/stagger開始点とする記述・受け入れ条件も限定的にsupersedeする。既定はenemy-free初回準備60000ms、3 combat wave各150000ms、wave 1/2後のrest各60000ms、総630000msである。この文書中の3分記述と旧run開始時initial/stagger記述は#71前の履歴であり、現在のrun時間・phase残り・victory境界はREQ-WAVE-PROGRESSIONを正本とする。60秒spawn phaseは初回combat開始をepochとし、`status=playing && elapsedMs >= restDurationMs`を満たす最初のupdateでcurrent phaseにかかわらず一度だけinitial/staggerを開始する。4方向と主9・反3の方向規則、terminal/retryのgeneration guard、弾薬箱respawn契約は維持する。

## 必須範囲

- 初回combat開始時をphase 0とし、combat epochからの経過時間60000msごとにphaseを1増やす。preparation中のHUD表示はphase 0だがenemyをspawnしない。phase表示は利用者向けに1始まりとする。
...
- Issue #38 follow-upでは初回combat epochで`basic-1`〜`basic-6`と`drone-1`〜`drone-2`の8体を初期対象とし、`basic-7`、`basic-8`、`basic-9`、`drone-3`を3000/6000/9000/12000msで段階投入してstable 12体へ到達させる。
...
- retryでは旧runの全enemy spawn/retry/death/recycle TimerEventと敵を破棄し、新しいrunの開始時刻、map、enemy-free preparation、phaseを初期化する。初期8体と段階投入は次の初回combat epochで開始する。
...
- Issue #34の180000ms生存勝利は#71前の履歴とし、victory/defeat時の停止、retry初期化、取得済み弾薬箱の30000ms後respawnを維持する。敵のphaseは初回combat epochを基準とし、terminal後に新しい敵をspawnまたはrespawnしない。弾薬箱respawn側もactiveな敵との非重複を維持する。

## 受け入れ条件

1. 初回combat epochから59999msまではphase 0であり、60000msの境界でphase 1へ進む。以後も60000msごとにphaseが進む。preparation中はphase 0表示かつenemy-freeである。
...
7. retry後はenemy-free preparationとphase 0表示、新map seedの主方向へ戻る。次の初回combat epochで初期8体と段階投入予約を一度だけ作り、旧generationの全TimerEventは副作用を与えない。
