...
| 箱復活 | src/main.ts のtile key、ammoBoxRespawns、元tile、static group |
...
1. run開始時にtimerを作成し、HUDへ05:00を表示する。
2. プレイヤーは既存の移動、射撃、リロード、箱取得を行う。
3. 箱取得成功時は元tileを記録した箱を削除し、同keyの300000ms timerを1つ作成する。
4. 300000ms境界ではvictoryへ遷移し、残り時間00:00とVictory UIを表示する。
...
survival timerとammo box復活timerはともに300000msで、run開始後に取得した箱の復活期限は通常survival victory後になる。victory/defeat/retryで復活timerを停止・clearするterminal優先を守り、terminal後にpending callbackから箱を再出現させない。
...
- 既に同じtile keyのtimerがある場合は追加登録せず、callback時にも同じtileのactive箱を重複生成しない。
...
小さなtile keyとMapだけを使い、汎用inventory・loot・persistence・通信・enemy time scaling・recovery itemを追加しない。#21/#35の実装、既存ammo/map配置文書、map生成規則は変更しない。期限の変更やterminal後も箱を復活させる仕様が必要になった場合は、Issue #34の条件を更新してから別sliceで再検討する。
