...
リロード処理は既存のタイマーと世代管理を再利用する。空弾倉への左クリックは射撃処理から既存 `reload()` へ委譲し、rules側の状態ガードと既存タイマー参照で重複開始を防ぐ。
