...
本設計は旧180000ms（03:00）survivalと3つの60000ms waveを、3 combat wave各150000msと間の2 rest各60000ms、既定570000ms（09:30）のscheduleへ置き換える。victory/defeatのterminal停止、retry、ammo box respawn、[REQ-DIRECTIONAL-SPAWN](../../20_requirements/directional-spawn-v1.md)の60000ms spawn phaseは維持する。restは表示、敵通常速度、recycle閾値、map配色だけを切り替え、active敵を停止、再配置、回復、置換しない。
...
flowchart TD
  ...
  state --> initial[既存のinitial 8をstrict spawn]
...
retryはgenerationを増やして旧callbackを無効化し、`retryCombat`と`retryRun(schedule)`を同時に作る。new map、initial 8、stagger timer、HUD、resultを既存経路で再構築する。RunState初期化時はelapsed 0、wave 1、combat、既定ならwave残り/phase残り02:30、goal 12、kills 0とし、active、current、remainingはstrict spawn成功済みslotの実数だけを表示する。
