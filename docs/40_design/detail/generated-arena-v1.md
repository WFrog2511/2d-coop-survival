...
- ワールドは 40px タイル、80×50 マス、3200×2000px とする。
...
| `e2e/prototype.spec.ts` | 800×500px canvasのDOM順序・初期化後の固定表示、map HUD、移動、再挑戦、既存の戦闘ループを代表経路として検査する。 |

## 4. 業務ルール

| ルールID | ルール | 根拠 | 未実装理由 |
|---|---|---|---|
| BR-ARENA-001 | 表示領域は 800×500px、ワールドは 80×50 タイル（3200×2000px）、1 タイルは 40px とする。 | プレイヤーの操作感を維持しつつ探索範囲を作る。 | なし |
...
| BR-ARENA-004 | 通路は 1〜3 タイル幅の L 字で部屋中心を順に接続する。通常部屋は14、幅8〜12、高さ6〜10タイルとし、最大8候補が不成立なら3列×3行の中央開始8部屋（各12×10タイル）を中央から上下左右へ幅2通路で接続する。 | 狭所と広所の最小の組合せを作る。 | なし |
| BR-ARENA-005 | 初期・再出現は他個体と重ならず、viewport 外の floor を優先する。 | 固定位置・固定時刻の出現を避ける。 | なし |
| BR-ARENA-006 | BFS は floor 上の 4 近傍だけを通過する。 | 80×50 のワールドに十分である。 | なし |
...
通常生成では 14 部屋（幅 8〜12、高さ 6〜10）を置く。各部屋は外周と他部屋から最低 1 タイルの wall 余白を保ち、候補配置は部屋ごとに最大 80 回試す。生成全体は最大 8 回まで次 seed を試す。上限到達時は、ワールド幅・高さから余白を導出した3列×3行の8位置から中央の開始部屋と7部屋を選び、中央から上下左右へ幅2通路を伸ばすfallback地形を返す。80×50では部屋起点をx=7/34/61、y=5/20/35とする。fallback は seed を HUD に表示し、無限再試行をしない。
...
2. 最初の矩形部屋はワールド中央帯で十分な余白を残して作り、その中心を player 開始 tile とする。
...
- camera bounds は `(0, 0, 3200, 2000)`、`startFollow(player)` を設定する。
...
- 汎用 `selectSpawnTile` は未占有 floor を選び、player の viewport に交差しない候補を優先する。viewport 外候補がないときだけ、player からの Manhattan 距離が最大の floor を座標順で決定的に選ぶ。
- 敵専用 `selectEnemySpawnTile` は未占有・viewport 外・割当方向・player から非可視かつ隣接 tile も非可視・到達可能な floor だけを候補にし、seed 付きで選ぶ。厳格候補がなければ `null` を返し、最遠 floor へ fallback しない。
...
地形生成に失敗しても fallback 地形で `active` へ進める。敵専用 spawn の厳格候補が不足した場合は重複を許さず body を無効化し、1秒後に同じ出現理由で再試行する。最遠 floor fallback は汎用 `selectSpawnTile` だけで用いる。HUD 要素を取得できない既存の起動失敗条件は維持する。
...
| `generateFallbackArenaMap` | pure function | seed | 3列×3行の中央開始・上下左右接続の連結済みfallback `ArenaMap` | fallback生成不能時は明示的に失敗 |
...
| `selectSpawnTile` | pure function | `ArenaMap`、`SpawnRequest`、seed 状態 | 未占有 floor（viewport 外優先、なければ最遠 floor）または `null` | floor 不足時は `null` |
| `selectEnemySpawnTile` | pure function | `ArenaMap`、`EnemySpawnRequest`、seed 状態 | 厳格な非可視・方向付き候補または `null` | 厳格候補不足時は `null` |
...
| 敵の厳格spawn候補不足 | `arena-spawn-unavailable` | enemy body を無効化する | 1秒後に同じ出現理由で再試行 |
...
| 観点 | 方針 |
|---|---|
| 性能 | 80×50=4000 タイル、最大enemy 12体、250ms 間隔の BFS に限定する。 |
...
| Unit | 通路幅とfallback | 通常の各接続で 1〜3 タイル幅の通路メタ情報を残し、その幅で floor が掘られる。fallbackは80×50内の3列×3行中央開始8部屋を上下左右へ幅2通路で連結し、12 slotの厳格な方向spawn候補を確保する。 |
| Unit | BFS | 4 近傍のみを通る最短経路、wall・範囲外時の空配列を返す。 |
| Unit | spawn | 汎用 `selectSpawnTile` は viewport 外・未占有 floor を優先し、候補不足時は最遠 floor fallback へ進む。敵専用 `selectEnemySpawnTile` は厳格候補不足時に `null` を返す。 |
...
| E2E | 移動と camera | 操作後に player tile 座標が変わり、Canvas は操作説明・HUD・敵HPテキストより上の800×500px表示を初期化後も維持する。 |
