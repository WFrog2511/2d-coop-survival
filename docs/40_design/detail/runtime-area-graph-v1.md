...
| 項目 | 内容 |
| --- | --- |
| 対象機能 | current topologyからの3×3 area / 2×2 corridor / 細いjunction graph導出 |
...
| 関連Issue | [Issue #100](https://github.com/WFrog2511/2d-coop-survival/issues/100)、[Issue #102](https://github.com/WFrog2511/2d-coop-survival/issues/102)、[Issue #104](https://github.com/WFrog2511/2d-coop-survival/issues/104) |
...
1. floor tileごとに、tileを含む3×3全floor blockがあるか確認する。あれば`area`。
2. area以外のfloorについて、tileを含む2×2全floor blockがあるか確認する。あれば`corridor`。
3. 2×2 blockにも参加しない細いfloorは4近傍floorを数え、3以上なら`junction`、それ以外は`corridor`。
4. row-major順の未訪問tileから同kindだけを4近傍探索し、componentを一nodeにする。
5. nodeが異なる隣接floorの接触を重複なくedgeにする。
...
`applyTopologyMutations()`は既存のatomic validation結果が同一snapshotなら直ちに戻り、graphもterrainも更新しない。新snapshotなら最初に`areaGraph`を再導出し、その後に既存のterrain rebuild、path/visibility cache reset、minimap、HUD更新を実行する。graph作成はPhaser objectに触れないため、失敗しない正常入力では既存runtime責務を広げない。`ArenaMap.rooms`は生成時metadataのままとし、3×3 `area`とのID対応や相互変換は持たない。
...
production向けのglobal API、graph固有UI、通信、保存は追加しない。音響debugがcurrent graphをworld/minimapへ投影しても、その表示は[acoustic-graph-v1 詳細設計](acoustic-graph-v1.md)の責務である。DEV E2Eだけが既存`window.__arenaScene`から`areaGraph`を読む。

## 4. テスト観点

| レベル | 観点 | 期待結果 |
| --- | --- | --- |
| unit | 3×3 area、area外の2×2 corridor、細いcross junction、同kind component | tileが一意の期待kind nodeへ割り当てられる |
...
| 2026-08-21 | 空間分類を3×3 areaと2×2 corridorへ同期 | 2tile幅の通路をroom相当に誤分類せず、細い分岐だけをjunctionへ残す |
