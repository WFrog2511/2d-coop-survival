...
- 武器: 1〜3でクイックスロットを選択（初期所持はライフル）。ショットガンは1クリック1射、ハンドガン・リボルバー・コンパクトピストルは同じ共有弾薬で1クリック1射
- 照準: マウス移動
- リロード: R（選択武器の残弾を補充。リロード中の射撃不可）
- pickup: 3x3近傍で表示される対象名・操作の2行E案内中に、弾薬箱・weapon model・スクラップを1件取得。空きクイックスロット、次にバックパックへ非stackで格納する
- 敵balance: 基本敵HP4。高速ドローンは高速だがHP2で、ライフル1発で撃破可能
- Canvas HUD: 上中央はWave・昼（準備）/夜（戦闘）/昼（休憩）・現在フェーズ残り、右上はキル数、player HPは左下、選択武器のammoは右下、武器3枠とスクラップは下部中央。world item総数はfieldへ表示しない。Tabで3クイックスロットと10バックパックをnonmodalに開くと、右隣にライフル・ショットガン・ハンドガンの予備弾を一覧するpouchも表示する。Tab中はmouse射撃を停止するが、移動、時間進行、`R`、pickup、slot選択は続く。武器を枠間でdrag & dropしてmove/swapでき、game領域へdropしたweaponは近傍floorへ置かれ、Eで再取得できる
- 再挑戦: 敗北表示の「再挑戦」ボタン
...
実際のブラウザ操作を表示する場合はheadedモードを使います。代表E2Eは、enemy-free初回準備、combat開始時の初期8体と3/6/9/12秒の段階投入、strict hidden spawn、combat epochからの60秒directional spawn phase、preparation/combat/rest境界と敵のcurrent phase respawn、時間/HP/quick slot/ammo overlayとpouchを観測しながら、visibility、移動、Tab中の射撃抑止、有限弾薬、リロード、弾薬箱respawn、weapon model・スクラップpickup、inventory move/swapとworld drop、10:30既定runの勝利、敗北、retryを確認します。
